...
import { enrichmentCards } from "./site-data";
      ...
      <div className="section-container max-w-[980px]">
        <div className="rounded-[26px] bg-[#fbfbfb] px-8 py-10 shadow-[0_18px_60px_rgba(15,23,42,0.06)] sm:px-10 sm:py-12">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {enrichmentCards.map((item, index) => {
              const Icon = icons[index];
              return (
                <article
                  key={item.name}
                  className={`relative overflow-hidden rounded-3xl border border-slate-200 bg-white px-5 py-6 shadow-[0_14px_34px_rgba(15,23,42,0.08)] before:absolute before:left-0 before:top-0 before:h-full before:w-[3px] ${item.accent}`}
                >
          ...
          <div className="mt-8 flex flex-col gap-6 rounded-[22px] border border-[#ccefed] bg-[linear-gradient(135deg,#efffff_0%,#e9fbff_48%,#e9f8f3_100%)] px-6 py-7 shadow-[0_12px_40px_rgba(43,104,122,0.08)] md:flex-row md:items-center md:justify-between">
            ...
            <Link
              href="/courses/"
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-brandGold px-6 py-3 text-sm font-bold text-black transition-opacity hover:opacity-90"
            >
              Learn More
        ...
        <Link
          href="/courses/"
          className="mt-10 inline-flex items-center justify-center gap-2 rounded-full bg-brandGold px-6 py-3 text-sm font-bold text-black transition-opacity hover:opacity-90"
        >
          Explore All Courses
          <ArrowRight className="h-4 w-4" />
        </Link>

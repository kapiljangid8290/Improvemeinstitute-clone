          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="hidden items-center gap-0.5 lg:flex">
              {homeNavLinks.map((item) =>
                item.label === "Our Courses" ? (
                  ...
                  >
                    <Link href={item.href} className="flex items-center gap-0.5 rounded-md px-4 py-3 text-sm font-medium text-navy-900 transition-colors hover:text-yellow-500">
                      {item.label}
                      <ChevronDown className={`h-4 w-4 transition-transform ${coursesOpen ? "rotate-180" : ""}`} />
                    </Link>
                    {coursesOpen ? (
                      <div className="absolute left-0 top-full pt-3">
                        <div className="w-[760px] rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_24px_70px_rgba(15,23,42,0.14)]">
                  ...
                  <Link
                    key={item.label}
                    href={item.href}
                    className={`rounded-md px-4 py-3 text-sm font-medium transition-colors hover:text-yellow-500 ${
            ...
            <div className="hidden lg:block">
              <Link
                ...
                className="inline-flex items-center justify-center rounded-lg bg-brandGold px-5 py-2.5 text-sm font-bold text-black transition-all duration-200 hover:opacity-95 hover:shadow-md"
              >
        ...
        {open ? (
          <div className="border-t border-slate-200 bg-white lg:hidden">
            <div className="section-container animate-slideIn py-4">
                ...
                {homeNavLinks.map((item) => (
                  <div key={item.label}>
                    {item.label === "Our Courses" ? (
                      ...
                      <Link href={item.href} onClick={() => setOpen(false)} className="block rounded-md px-3 py-3 text-sm font-medium text-navy-900 transition-colors hover:bg-slate-50 hover:text-yellow-600">
                        {item.label}
                      </Link>
                    )}
                  </div>
              ...
              <Link href="/contact/#booking-form" onClick={() => setOpen(false)} className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-brandGold px-5 py-3 text-sm font-bold text-black transition-opacity hover:opacity-90">
                Book Free Assessment
              </Link>
            </div>
          </div>
